    ...
    public sealed partial class MainPage : Page
    {
        ...
        private async Task SetParamText()
        {
            ...
            while (true)
            {
                ...
                GetOxyLevel();
                if (Parameters.pulse < 1)
                {
                    Parameters.pulse = 0;
                    Parameters.systolicBloodPressure = 0;
                    Parameters.diastolicBloodPressure = 0;
                    return;
                }
                temperature.Text = Parameters.temperature.ToString();
                bloodPress.Text = Parameters.systolicBloodPressure.ToString() + "/" + Parameters.diastolicBloodPressure.ToString();
        ...
        private void GetOxyLevel()
        {
            ...
            if (oxyLevel.Value >= 0)
                ...
                oxyLevel.Value = 0;
            if (oxyLevel.Value < 30)
                Parameters.pulse += rand.Next(1, 2);
            else if (oxyLevel.Value >= 30 && oxyLevel.Value < 60)
                Parameters.pulse += rand.Next(0, 1);
            

            if (Parameters.oxygenStatus == 0 && Parameters.pulse > 0) 
            {
                Parameters.pulse -= rand.Next(5, 8);
                if (Parameters.systolicBloodPressure <= 0 || Parameters.diastolicBloodPressure <= 0)
                {
                    Parameters.systolicBloodPressure = 0;
                    Parameters.diastolicBloodPressure = 0;
                }
                Parameters.systolicBloodPressure -= rand.Next(4, 7);
                Parameters.diastolicBloodPressure -= rand.Next(3, 5);
